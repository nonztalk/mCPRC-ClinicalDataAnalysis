...
for (i in 1:25) {
  CELinCohortCV <- rbind(CELinCohortCV, reshapeData(kFoldCV(habini_CEL_death, 5, i)))
  VENinCohortCV <- rbind(VENinCohortCV, reshapeData(kFoldCV(habini_VEN_death, 5, i)))
  ASCinCohortCV <- rbind(ASCinCohortCV, reshapeData(kFoldCV(habini_ASC_death, 5, i)))
}
...
# ASC + VEN ------------------ 
train <- rbind(habini_ASC_death, habini_VEN_death)
train$DISCONT <- as.numeric(as.character(train$DISCONT))
train <- na.omit(train)
test <- habini_CEL_death
test <- test[!grepl("\\.", test$DISCONT), ]
test$DISCONT <- as.numeric(as.character(test$DISCONT))
...
# CEL + VEN ------------------ 
train <- rbind(habini_VEN_death, habini_CEL_death)
train$DISCONT <- as.numeric(as.character(train$DISCONT))
train <- na.omit(train)
test <- habini_ASC_death
test <- test[!grepl("\\.", test$DISCONT), ]
test$DISCONT <- as.numeric(as.character(test$DISCONT))
...
# ----------------
# 2. try to normalize the combined data with scale()
# ASC + CEL --> VEN ---------------------
train <- rbind(habini_ASC_death, habini_CEL_death)
train$DISCONT <- as.numeric(as.character(train$DISCONT))
train <- na.omit(train)
...
# -------------------
## Try to use the dataset involve early death
# -------------------
# CV InCohort ------------------
...
# Feature Importance In Cohort ------------------
kFold <- function(data, fold, seed) {
  set.seed(seed)
  data <- data[sample(nrow(data)), ]
  folds <- cut(seq(1, nrow(data)), breaks = fold, labels = F)
  ...
  for (i in 1:fold) {
    ...
    rf_death <- randomForest(as.factor(DEATH) ~ . - (DISCONT + LKADT_P), data = train)
    rf_discont <- randomForest(as.factor(DISCONT) ~ . - (DEATH + LKADT_P), data = train)
    
    temp <- as.data.frame(rbind(rf_death$importance, rf_discont$importance))
    temp$Features <- row.names(temp)
    temp$GoldStandard <- rep(c("Death", "Discontinuation"), each = 9)
...
CELinCohortFI <- data.frame()
VENinCohortFI <- data.frame()
ASCinCohortFI <- data.frame()

for (i in 1:25) {
  CELinCohortFI <- rbind(CELinCohortFI, kFold(habini_CEL_death, 5, i))
  VENinCohortFI <- rbind(VENinCohortFI, kFold(habini_VEN_death, 5, i))
  ASCinCohortFI <- rbind(ASCinCohortFI, kFold(habini_ASC_death, 5, i))
}

p <- ggplot(CELinCohortFI)
p <- p + geom_boxplot(aes(Features, MeanDecreaseGini), width = 0.5) + coord_flip() + theme_bw() + facet_grid(.~GoldStandard) + 
  labs(y = "Importance", title = "Feature Importance -- CEL")
p1 <- ggplot(VENinCohortFI)
p1 <- p1 + geom_boxplot(aes(Features, MeanDecreaseGini), width = 0.5) + coord_flip() + theme_bw() + facet_grid(.~GoldStandard) + 
  labs(y = "Importance", title = "Feature Importance -- VEN")
p2 <- ggplot(ASCinCohortFI)
p2 <- p2 + geom_boxplot(aes(Features, MeanDecreaseGini), width = 0.5) + coord_flip() + theme_bw() + facet_grid(.~GoldStandard) + 
  labs(y = "Importance", title = "Feature Importance -- ASC")

grid.arrange(p, p1, p2, nrow = 3, ncol = 1)
...
rf <- randomForest(as.factor(DEATH) ~ . - (DISCONT + LKADT_P), data = train)
rf_discont <- randomForest(as.factor(DISCONT) ~ . - (DEATH + LKADT_P), data = train)

data_for_plot <- as.data.frame(rbind(rf$importance, rf_discont$importance))
data_for_plot$Features <- row.names(data_for_plot)
data_for_plot$GoldStandard <- rep(c("Death", "Discontinuation"), each = 9)
p <- ggplot(data_for_plot)
p <- p + geom_bar(aes(Features, MeanDecreaseGini), stat = "identity", alpha = 0.6, position = "dodge", width = 0.5) + 
  theme_bw() + coord_flip() + labs(y = "Importance", title = "Feature Importance -- ASC + CEL") + facet_grid(.~GoldStandard)
...
data_for_plot$Features <- row.names(data_for_plot)
data_for_plot$GoldStandard <- rep(c("Death", "Discontinuation"), each = 9)
p1 <- ggplot(data_for_plot)
p1 <- p1 + geom_bar(aes(Features, MeanDecreaseGini), stat = "identity", alpha = 0.6, position = "dodge", width = 0.5) + 
  theme_bw() + coord_flip() + labs(y = "Importance", title = "Feature Importance -- ASC + VEN") + facet_grid(.~GoldStandard)

# CEL + VEN ---------------
train <- rbind(habini_CEL_death, habini_VEN_death)
train$DISCONT <- as.numeric(as.character(train$DISCONT))
train <- na.omit(train)
test <- habini_ASC_death
test <- test[!grepl("\\.", test$DISCONT), ]
...
data_for_plot$Features <- row.names(data_for_plot)
data_for_plot$GoldStandard <- rep(c("Death", "Discontinuation"), each = 9)
p2 <- ggplot(data_for_plot)
p2 <- p2 + geom_bar(aes(Features, MeanDecreaseGini), stat = "identity", alpha = 0.6, position = "dodge", width = 0.5) + 
  theme_bw() + coord_flip() + labs(y = "Importance", title = "Feature Importance -- VEN + CEL") + facet_grid(.~GoldStandard)

grid.arrange(p, p1, p2, nrow = 3, ncol = 1)
tiff("./Figure for paper/FeatureImportance_CrossCohort.tiff", width = 641, height = 1500)
dev.off()
